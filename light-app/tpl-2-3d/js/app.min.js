define("lib/iscroll/iscroll", [], function(a, b, c) {
...
}), define("modules/adress/main", ["units/yunlaiMap", "lib/zepto/zepto", "lib/zepto/selector", "lib/zepto/touch", "lib/zepto/data", "http://api.map.baidu.com/getscript?v=1.4&ak=h6rMzpaZgIOMvdzzGEzcGgBk&services=true&t=20140320104737", "modules/app/main", "units/lightAppAd"], function(require, exports, module) {
	$ = require("units/yunlaiMap");
	var app = require("modules/app/main"),
		$adressPages = $(".adress");
	module.exports = {
		init: function() {
			$adressPages.each(function(i, item) {
				var $page = $(item);
				console.log("adress init"), $page.on("active", function() {
					console.log("adress active")
				}).on("current", function() {
					console.log("adress current")
				});
				...
				$(map).on("show", function() {
					...
					app.enableFlipPage()
				}), $page.on($.isPC ? "click" : "tap", "a.linkOpen", function(e) {
					try {
						...
						if (markers)
							for (var markersArr = eval(markers), i = 0; i < markersArr.length; i++) {
								...
								marker.title = marker.name, marker.content = marker.tel ? '电话：<a href="tel:' + marker.tel + '">' + marker.tel + "</a><br/>地址：" + marker.addr : "地址：" + marker.addr, delete marker.name, delete marker.tel, delete marker.addr, delete marker.desc, marker.content = marker.content.replace(/(\<br\s*\/\>\s*)+/gi, "<br/>"), map.addMarker(marker)
							}
					} catch (e) {
						console.log(e)
					}
...
}), define("modules/app/main", ["lib/zepto/zepto", "lib/zepto/selector", "units/lightAppAd", "touch", "system/util/objectUtil"], function(require, exports, module) {
	function getNum(a) {
		return Number(a.replace(/[^-0-9]/gi, "")) || 0
	}
	var $ = require("lib/zepto/zepto"),
		$ = require("lib/zepto/selector");
	require("units/lightAppAd");
	var App = function(a) {
		console.log("app init"), this._$app = a, this._$pages = this._$app.find(".page"), this.$currentPage = this._$pages.eq(0), this._isFirstShowPage = !0, this._isInitComplete = !1, this._isDisableFlipPage = !1;
		var b = this,
			c = 0,
			d = document.getElementById("cube"),
			e = $(window);
		! function() {
			e.on("scroll.elasticity", function(a) {
				a.preventDefault()
			}).on("touchmove.elasticity", function(a) {
				a.preventDefault()
			}), e.delegate("img", "mousemove", function(a) {
				a.preventDefault()
			})
		}(), e.on("load", function() {
			var a = null,
				...
				q = 0,
				r = 0;
			b._$app.on("mousedown touchstart", "section.page", function(o) {
				loaded || b._isDisableFlipPage || (a = $(this), e = null, a && (l = !0, m = !1, n = !0, j = 0, k = 0, "mousedown" == o.type ? (h = o.pageX, i = o.pageY) : (h = o.touches[0].pageX, i = o.touches[0].pageY), c = getNum(d.style.webkitTransform), g = a.prev(), g.length < 1 && (g = a.siblings().last()), f = a.next(), f.length < 1 && (f = a.siblings().first()), a.addClass("z-move")))
			}).on("mousemove touchmove", "section.page", function(s) {
				if (l && (e || n) && ("mousemove" == s.type ? (j = s.pageX - h, k = s.pageY - i) : (j = s.touches[0].pageX - h, k = s.touches[0].pageY - i), Math.abs(j) > Math.abs(k)))
					if (b._isDisableFlipPage = !0, j > 0)
						if (m || n)
							if (m = !0, p = !0, n = !1, e && e.removeClass("z-active z-move"), e = a.prev(), e.length < 1 && (e = a.siblings().last()), e) {
								if (e.addClass("z-active z-move"), q = 90 / (window.innerWidth / j), d.style.webkitTransform = "rotateY(" + (c + q) + "deg)", o) {
									if (a.hasClass("front")) return void g.addClass("left");
									if (a.hasClass("left")) return void g.addClass("back");
									if (a.hasClass("back")) return void g.addClass("right");
									if (a.hasClass("right")) return void g.addClass("front");
									e.trigger("active"), o = !1
								}
							} else d.style.webkitTransform = "rotateY(" + c + "deg)";
				else d.style.webkitTransform = "rotateY(" + c + "deg)";
				else if (0 > j)
					if (!m || n)
						if (m = !1, o = !0, n = !1, e && e.removeClass("z-active z-move"), e = a.next(), e.length < 1 && (e = a.siblings().eq(0)), e) {
							if (e.addClass("z-active z-move"), r = 90 / (window.innerWidth / j), d.style.webkitTransform = "rotateY(" + (c + r) + "deg)", p) {
								if (a.hasClass("front")) return void f.addClass("right");
								if (a.hasClass("left")) return void f.addClass("front");
								if (a.hasClass("back")) return void f.addClass("left");
								if (a.hasClass("right")) return void f.addClass("back");
								e.trigger("active"), p = !1
							}
						} else d.style.webkitTransform = "rotateY(" + c + "deg)";
				else d.style.webkitTransform = "rotateY(" + c + "deg)"
			}).on("mouseup touchend", "section.page", function() {
				if (l)
					if (l = !1, p = !0, o = !0, e) b._isDisableFlipPage = !0, Math.abs(j) > Math.abs(k) && Math.abs(j) > 100 ? m ? $(d).animate({
						rotateY: c + 90 + "deg"
					}, 400, function() {
						a.removeClass("front left back right z-current z-move"), g.hasClass("z-active") || g.removeClass("front back right left"), f.hasClass("z-active") || f.removeClass("front back right left"), e.removeClass("z-active z-move"), e.addClass("z-current"), setTimeout(function() {
							b._isDisableFlipPage = !1
						}, 100), e.trigger("current")
					}) : $(d).animate({
						rotateY: c - 90 + "deg"
					}, 400, function() {
						a.removeClass("front left back right z-current z-move"), g.hasClass("z-active") || g.removeClass("front back right left"), f.hasClass("z-active") || f.removeClass("front back right left"), e.removeClass("z-active z-move"), e.addClass("z-current"), setTimeout(function() {
					...
					}) : (d.style.webkitTransform = "rotateY(" + c + "deg)", setTimeout(function() {
						g.removeClass("front back right left z-active z-move"), f.removeClass("front back right left z-active z-move"), b._isDisableFlipPage = !1
					}, 500));
					else if (Math.abs(k) > Math.abs(j) && Math.abs(k) > 120) {
					var h = $(".app-footer .bubble");
					g.removeClass("front back right left z-active z-move"), f.removeClass("front back right left z-active z-move"), b._isDisableFlipPage = !1, h.addClass("show"), setTimeout(function() {
						h.remove()
					}, 2e3)
				} else d.style.webkitTransform = "rotateY(" + c + "deg)", a.removeClass("z-move")
			})
		})
	};
	App.prototype.showPage = function(a) {
		...
		window._app_showPage ? window._app_showPage : window._app_showPage = function(a) {
			...
			switch (c) {
				...
				case "object":
					b = $(a)
			}!this._isFirstShowPage || b && b.length || (b = this.$currentPage, this._isFirstShowPage = !1), b && b.length && (this._$pages.filter(".z-current").removeClass("z-current"), this.$currentPage = b)
...
}), define("modules/app/statistics", [], function(a, b, c) {
	var d = $("body").data("app-id"),
		e = {
			behaviorStatistics: function(a) {
				$.ajax({
					url: "/analyseplugin/plugin?activity_id=" + d + "&plugtype=" + a,
					type: "get"
				})
			},
			multiStatistics: function() {
				var a = location.search.substr(location.search.indexOf("channel=") + 8);
				a = a.match(/^\d+/), (!a || isNaN(a) || 0 > a) && (a = 1), $.ajax({
					type: "POST",
					cache: !1,
					url: "/simple/analyse/" + d + "?channel=" + a,
					data: {
						channel: a
					},
					dataType: "json",
	...
	e.multiStatistics(), c.exports = e
}), define("modules/index/form", ["units/maskLayer", "selector", "touch", "data", "lib/iscroll/iscroll"], function(a) {
	$ = a("units/maskLayer");
	var b = a("lib/iscroll/iscroll"),
		c = $(".form");
	c.each(function(a, c) {
		function d(a, b) {
			var c = $('[name="name"]').prop("type");
			return "radio" != c && "checkbox" != c && (a.data("value", a.val()).val(b).addClass("z-error"), a.blur()), h.val("请填写完整").prop("disabled", !0), !1
		}
		var e = $(c),
			f = e.find(".m-contactForm");
		if (f.length) {
			var g = f.find("#formContact"),
				h = g.find(".btn-submit"),
				i = e.find(".m-Layer").maskLayer(),
				j = f.find(".successTipLayer").maskLayer({
					closeButton: !1
				}),
				k = i.maskLayer("getPluginObject"),
				l = j.maskLayer("getPluginObject");
			if (k.on("show", function() {
				$(".u-globalAudio").hide(), app.disableFlipPage()
			}), k.on("hide", function() {
				$(".u-globalAudio").show(), app.enableFlipPage(), g[0].reset(), h.prop("disabled", !1)
			}), e.find("a.linkOpen").on("click tap", function() {
				i.maskLayer("show")
			}), h = g.find(".btn-submit"), !window.form_contactFormLayer_iScrollFloag) {
				window.form_contactFormLayer_iScrollFloag = !0;
				var m = $(window),
					n = new b(i[0]);
				g.delegate("input", "focus", function() {
					form_focus_input = this, setTimeout(function(a) {
						n.refresh(), n.scrollToElement(a)
					}, 100, form_focus_input)
				}), m.on("resize", function() {
					f.css("margin-top", 120), setTimeout(function() {
						f.css("margin-top", 0), f.parent().css({
							"margin-top": 120,
							"padding-bottom": 120
						})
					}, 100), window.navigator.userAgent.indexOf("iPhone") >= 0 && i.css("height", window.innerHeight)
				}).resize()
			}
			g.delegate("input.z-error", "focus", function() {
				var a = $(this);
				a.val(a.data("value")).removeClass("z-error"), h.val("提 交").prop("disabled", !1)
			}).delegate(".btn-submit", $.isPC ? "click" : "tap", function() {
				h.prop("disabled") || g.submit()
			}), g.on("submit", function(a) {
				a.preventDefault();
				var b = g.find('input[name="name"]'),
					c = (g.find('input[name="sex"]'), g.find('input[name="tel"]')),
					e = g.find('input[name="company"]'),
					f = g.find('input[name="post"]'),
					h = g.find('input[name="email"]');
				if (b.length && 0 == $.trim(b.val()).length) return d(b, "请输入姓名！");
				if (c.length && 0 == $.trim(c.val()).length) return d(c, "请输入电话！");
				if (c.length > 0 && $.trim(c.val()).length > 0) {
					var i = /^13[0-9]{9}|15[0-9]{9}|17[0-9]{9}|18[0-9]{9}$/;
					if (!$.trim(c.val()).match(i)) return d(c, "电话号码输入不正确！")
				}
				if (h.length && 0 == $.trim(h.val()).length) return d(h, "请输入邮箱！");
				if (h.length > 0 && $.trim(h.val()).length > 0) {
					var i = /(^[a-z\d]+(\.[a-z\d]+)*@([\da-z](-[\da-z])?)+(\.{1,2}[a-z]+)+$)/i;
					if (!$.trim(h.val()).match(i)) return d(h, "邮箱格式不正确！")
				}
				return e.length && 0 == $.trim(e.val()).length ? d(e, "请输入公司名称！") : f.length && 0 == $.trim(f.val()).length ? d(f, "请输入职务！") : void $.ajax({
					url: g.attr("action"),
					type: g.attr("method"),
					data: g.serialize(),
					dataType: "json",
...
window.setTimeout = function(a, b) {
	...
	__sto(d, b)
}, define("modules/index/main", ["units/maskLayer", "selector", "touch", "data", "lib/iscroll/iscroll"], function(a, b, c) {
	var d = $(".form");
	$ = a("units/maskLayer");
	...
	c.exports = {
		init: function() {
			d.each(function(a, b) {
				function c(a, b) {
					var c = $('[name="name"]').prop("type");
					return "radio" != c && "checkbox" != c && (a.data("value", a.val()).val(b).addClass("z-error"), a.blur()), $btnSubmit.val("请填写完整").prop("disabled", !0), !1
				}
				var d = $(b);
				console.log("form init");
				var f = d.find("a.linkOpen");
				if (f.length) {
					var g = d.find(".m-contactForm"),
						h = g.find("#formContact"),
						i = d.find(".m-Layer").maskLayer(),
						j = g.find(".successTipLayer").maskLayer({
							closeButton: !1
						}),
						k = i.maskLayer("getPluginObject"),
						l = j.maskLayer("getPluginObject");
					if (i.maskLayer({
						...
						onHide: function() {
							$(".u-globalAudio").show(), app.enableFlipPage(), h[0].reset(), $btnSubmit.prop("disabled", !1)
						}
					}), f.on($.isPC ? "click" : "tap", function() {
						i.maskLayer("show")
					}), $btnSubmit = h.find(".btn-submit"), !window.form_contactFormLayer_iScrollFloag) {
						window.form_contactFormLayer_iScrollFloag = !0;
						var m = $(window),
							n = h.find($("input").first()),
							o = new e(i[0]);
						h.delegate("input", "focus", function() {
							n = this, setTimeout(function(a) {
								o.refresh(), o.scrollToElement(a)
							}, 100, n)
						}), m.on("resize", function() {
							g.css("margin-top", 120), setTimeout(function() {
								g.css("margin-top", 0), g.parent().css({
									"margin-top": 120,
									"padding-bottom": 120
								})
							}, 100), i.css("height", window.innerHeight), setTimeout(function(a) {
								o.refresh(a), o.scrollToElement(a)
							}, 100, n[0])
						}).resize()
					}
					h.delegate("input.z-error", "focus", function() {
						var a = $(this);
						a.val(a.data("value")).removeClass("z-error"), $btnSubmit.val("提 交").prop("disabled", !1)
					}).delegate(".btn-submit", $.isPC ? "click" : "tap", function() {
						$btnSubmit.prop("disabled") || h.submit()
					}), h.on("submit", function(a) {
						a.preventDefault();
						var b = h.find('input[name="name"]'),
							d = (h.find('input[name="sex"]'), h.find('input[name="tel"]')),
							e = h.find('input[name="company"]'),
							f = h.find('input[name="post"]'),
							g = h.find('input[name="email"]');
						if (b.length && 0 == $.trim(b.val()).length) return c(b, "请输入姓名！");
						if (d.length && 0 == $.trim(d.val()).length) return c(d, "请输入电话！");
						if (d.length > 0 && $.trim(d.val()).length > 0) {
							var i = /^[0-9]{3,18}$/;
							if (!$.trim(d.val()).match(i)) return c(d, "电话号码输入不正确！")
						}
						if (g.length && 0 == $.trim(g.val()).length) return c(g, "请输入邮箱！");
						if (g.length > 0 && $.trim(g.val()).length > 0) {
							var i = /(^[a-z\d]+(\.[a-z\d]+)*@([\da-z](-[\da-z])?)+(\.{1,2}[a-z]+)+$)/i;
							if (!$.trim(g.val()).match(i)) return c(g, "邮箱格式不正确！")
						}
						return e.length && 0 == $.trim(e.val()).length ? c(e, "请输入公司名称！") : f.length && 0 == $.trim(f.val()).length ? c(f, "请输入职务！") : void $.ajax({
							url: h.attr("action"),
							type: h.attr("method"),
							data: h.serialize(),
							dataType: "json",
							success: function() {
								l.show(), setTimeout(function() {
									l.hide(), setTimeout(function() {
										app.enableFlipPage(), k.hide(), h[0].reset()
									}, 800)
								}, 2e3)
							},
							error: function() {
...
}), define("modules/link/main", [], function(a, b, c) {
	var d = $(".link");
	c.exports = {
		init: function() {
			d.each(function(a, b) {
...
}), define("modules/phone/main", ["units/maskLayer", "selector", "touch", "data", "lib/iscroll/iscroll"], function(a, b, c) {
	var d = $(".phone");
	$ = a("units/maskLayer");
	a("lib/iscroll/iscroll");
	c.exports = {
		init: function() {
			d.each(function(a, b) {
				var c = $(b);
				console.log("adress init"), c.on("active", function() {
...
}), define("modules/share/main", [], function(a, b, c) {
	var d = $(".share");
	c.exports = {
		init: function() {
			d.each(function(a, b) {
				...
				var d = c.find("a.linkOpen");
				if (d.length) {
					var e = c.find(".u-maskLayer");
					c.find(".page-content").append(e), e.maskLayer({
						clickHideMode: 2,
						closeButton: !1,
						onShow: function() {
							$(".u-globalAudio").hide(), app.disableFlipPage()
						...
						onHide: function() {
							$(".u-globalAudio").show(), app.enableFlipPage()
						}
					}), d.on($.isPC ? "click" : "tap", function() {
						e.maskLayer("show")
					})
				}
			})
...
}), define("modules/video/main", ["modules/app/main", "lib/zepto/zepto", "lib/zepto/selector", "units/lightAppAd", "units/maskLayer", "selector", "touch", "data", "./youkuVideo", "lib/zepto/touch", "lib/zepto/data", "lib/youku/jsapi", "units/globalAudio", "lib/zepto/coffee", "system/util/objectUtil"], function(a, b, c) {
	var d = a("modules/app/main"),
		e = a("units/maskLayer"),
		e = a("./youkuVideo"),
			...
			g.each(function(a, b) {
				var c, g = e(b),
					...
					k = g.find(".m-youkuVideoLayer").maskLayer({
						onShow: function() {
							h.hide(), e(".u-globalAudio").hide(), d.disableFlipPage(), j = f.playState, f.pause(), f.isAllowManually = !1, c = i.youkuVideo().youkuVideo("getPluginObject")
						},
						onHide: function() {
							h.show(), e(".u-globalAudio").show(), d.enableFlipPage(), "playing" == j && f.play(), f.isAllowManually = !0, c.destroy()
						}
					}).maskLayer("getPluginObject");
				h.on(e.isPC ? "click" : "tap", function() {
					k.show()
...
}), define("modules/video/youkuVideo", ["lib/zepto/touch", "lib/zepto/zepto", "lib/zepto/selector", "lib/zepto/data", "lib/youku/jsapi"], function(a, b, c) {
	$ = a("lib/zepto/touch"), $ = a("lib/zepto/selector"), $ = a("lib/zepto/data");
	var d = a("lib/youku/jsapi");
	! function() {
		var a = 0,
			b = function(b, c) {
				var e = this;
				this.$target = b.addClass("m-youkuVideo"), this.settings = null, this.player = null, this._playerID = "videoBody_" + ++a;
				var f = this.$target.data("devid"),
					g = this.$target.data("src");
				this.settings = $.extend({
					devID: f ? f : "168eed9e805f5239",
				...
				}, c), this.$target.attr("id", this._playerID), this.player = new d.YKU.Player(this._playerID, {
					styleid: "0",
					...
					autoplay: !0,
					events: {
						onPlayerReady: e.settings.onPlayerReady,
...
}), define("modules/words/iScroll", ["units/widget"], function(a, b, c) {
	function d(a, b, c) {
	...
	var e = a("units/widget");
	d.prototype = $.extend({}, e, {
		handleEvent: function(a) {
			switch (a.type) {
				case "touchstart":
				case "mousedown":
...
}), define("modules/words/main", ["lib/zepto/data", "lib/zepto/zepto", "units/widget", "./word", "lib/zepto/touch", "./iScroll"], function(a, b, c) {
	$ = a("lib/zepto/data");
	var d = (a("units/widget"), a("./word")),
		e = $(".words");
	c.exports = {
		init: function() {
			e.each(function(a, b) {
...
}), define("modules/words/word", ["lib/zepto/touch", "lib/zepto/zepto", "lib/zepto/data", "units/widget", "modules/words/iScroll"], function(require, exports, module) {
	$ = require("lib/zepto/touch"), $ = require("lib/zepto/data");
	var widget = require("units/widget"),
		iScroll = require("modules/words/iScroll"),
		words = function() {
	...
	words.prototype = $.extend({}, widget, {
		...
		wallOpen: function() {
			...
			this.openWallBtn.on(this._click, function() {
				...
				a.plugin.refresh(c - 60), app.disableFlipPage()
			})
		},
		wallClose: function() {
			var a = this;
					...
					that.wordsWall.data("ajaxSubmit", "true"), $.ajax({
						...
						success: function(msg) {
							if (that.loadingPageHide($(".u-pageLoading")), that.wordsWall.data("ajaxSubmit", "false"), msg.success) {
								...
								that.wodrdsDetail.find("ul").prepend(li), $("li[data-comment-id='" + id + "']").children("h4").children("span").text(name), $("li[data-comment-id='" + id + "']").children("p").text(txt);
		...
		wordsVerify: function(a, b) {
			var c = a.find("input"),
				...
				f = d.val(),
				g = c.val();
			return "" == $.trim(f) ? (b.showCheckMessage(e, "请对母校说点什么吧!", !1), d.focus(), !1) : "" == $.trim(g) ? (b.showCheckMessage(e, "留个名字，让母校知道你是哪位学生!", !1), c.focus(), !1) : f.length >= 140 && "" != f ? (b.showCheckMessage(e, "评论最多只能容纳140个字哦！", !1), d.focus(), !1) : g.length >= 30 && "" != g ? (b.showCheckMessage(e, "名字不能太长哦，30个字", !1), c.focus(), !1) : !0
		},
		event: function(a, b) {
			a._on("move", function() {
		...
		loadMoreCommend: function(obj) {
			...
			var count = 10,
				...
				layout_id = obj.wodrdsDetail.parents(".page").data("layout-id"),
				app_id = $("body").data("app-id"),
				url = "/simple/getcomment";
			$.ajax({
				url: url,
				cache: !1,
				dataType: "json",
				...
				success: function(msg) {
					if (msg.success) {
						for (var data = eval("(" + msg.data + ")"), i = 0; i < data.length; i++) {
							var h = $("<h4><span>" + data[i].from + "</span><strong>发表于" + data[i].date + "</strong></h4>"),
								p = $("<p>" + data[i].content + "</p>"),
								li = $('<li class="detail-item" data-comment-id="' + data[i].id + '"></li>').append(h).append(p);
...
}), define("page/init", ["modules/app/main", "lib/zepto/zepto", "lib/zepto/selector", "units/lightAppAd", "modules/index/main", "units/maskLayer", "lib/iscroll/iscroll", "modules/adress/main", "units/yunlaiMap", "modules/link/main", "modules/video/main", "modules/video/youkuVideo", "units/globalAudio", "modules/share/main", "modules/words/main", "data", "units/widget", "modules/words/word", "modules/phone/main", "units/weixin", "touch", "system/util/objectUtil", "lib/zepto/coffee", "modules/app/statistics"], function(a) {
	app = a("modules/app/main"), a("modules/index/main").init(), a("modules/adress/main").init(), a("modules/link/main").init(), a("modules/video/main").init(), a("modules/share/main").init(), a("modules/words/main").init(), a("modules/phone/main").init(), $ = a("units/weixin");
	a("units/lightAppAd"), a("units/globalAudio"), a("modules/app/statistics");
	$(window).on("load", function() {
		loaded = !0, $(".page").css("height", window.innerHeight).first().addClass("front z-current").trigger("current");
		var a = $(".page").length;
		if (a >= 4) {
			var b = $(".page").filter(".front"),
				c = b.next().addClass("right"),
				d = c.next().addClass("back"),
				e = d.next().addClass("left");
			setTimeout(function() {
				$("#cube").animate({
					rotateY: "-360deg"
				}, 2e3, function() {
					e.removeClass("left"), d.removeClass("back"), c.removeClass("right"), loaded = !1
				})
			}, 1500)
		} else loaded = !1;
		var f = $("#app-loading");
		f.addClass("z-hide"), f.on("webkitTransitionEnd", function() {
			f.remove()
		}), app._isInitComplete = !0
	...
	c.exports = d
}), define("units/globalAudio", ["lib/zepto/touch", "lib/zepto/zepto", "lib/zepto/coffee", "system/util/objectUtil"], function(a, b, c) {
	$ = a("lib/zepto/touch"), $ = a("lib/zepto/coffee");
	var d = a("system/util/objectUtil"),
		e = function(a) {
			this._$globalAudio = a, this._$tip = $("<span></span>"), this.audio = this._$globalAudio.find("audio")[0], this.isAllowManually = !1, this.playState = "ready";
			var b = this;
			this._$globalAudio.append(this._$tip), this._$globalAudio.coffee({
				...
				b.isAllowManually = !0
			}), this._$globalAudio.on($.isPC ? "click" : "tap", function(a) {
	...
	}, e.prototype._showTip = function(a) {
		var b = this;
		this._$tip.text(a), this._$tip.addClass("z-show"), setTimeout(function() {
			b._$tip.removeClass("z-show")
		}, 1e3)
	};
	var f, g = $(".u-globalAudio");
	f = g.length ? new e($(".u-globalAudio")) : d.createEmptyObject(e), c.exports = f
}), define("units/lightAppAd", ["lib/zepto/touch", "lib/zepto/zepto", "system/util/objectUtil"], function(a, b, c) {
	$ = a("lib/zepto/touch");
	var d = a("system/util/objectUtil"),
		e = function() {
	...
	e.prototype._init = function() {
		...
		if ($(".m-lightAppAd").length <= 0) {
			a.$lightAppAd = $('<div class="m-lightAppAd"><div><a href="javascript:void(0);" class="m-lightAppAd-link-guide"></a></div><div class="m-lightAppAd-body"><div class="m-lightAppAd-title"></div><a href="http://mp.weixin.qq.com/s?__biz=MjM5NTI3Mzk0MA==&mid=200206728&idx=1&sn=8edd0353287f322d1279e0d4eb581c2d#rd" class="m-lightAppAd-link-get"></a><a href="tel:4000168906" class="m-lightAppAd-link-tel"></a></div></div>'), $(".page:last").find(".page-content").append(a.$lightAppAd);
			var b = a.$lightAppAd.find(".m-lightAppAd-link-guide"),
				c = a.$lightAppAd.find(".m-lightAppAd-body");
			b.on($.isPC ? "click" : "tap", function(b) {
				a.$lightAppAd.addClass("z-showBody"), app.disableFlipPage(), b.stopPropagation()
			}), a.$lightAppAd.on($.isPC ? "click" : "tap", function() {
				a.$lightAppAd.removeClass("z-showBody"), app.enableFlipPage()
			}), c.on($.isPC ? "click" : "tap", function(a) {
				a.stopPropagation()
			})
		}
	...
	var f, g = $("body").data("ad");
	f = void 0 == g || 1 == g || "true" == g ? new e : d.createEmptyObject(e), c.exports = f
}), define("units/maskLayer", ["lib/zepto/selector", "lib/zepto/zepto", "lib/zepto/touch", "lib/zepto/data"], function(a, b, c) {
	$ = a("lib/zepto/selector"), $ = a("lib/zepto/touch"), $ = a("lib/zepto/data"),
	function() {
		var a = function(a, b) {
			var c = this;
			if (this._$maskLayer = a.addClass("u-maskLayer"), this.state = this._$maskLayer.is(".z-show") ? "show" : "hide", this.settings = $.extend({
				clickHideMode: 1,
				closeButton: !0,
				onShow: function() {},
				onHide: function() {}
				...
				var d = $('<a href="javascript:void(0);" class="u-maskLayer-close"></a>');
				c._$maskLayer.append(d), d.on($.isPC ? "click" : "tap", function(a) {
					a.preventDefault(), c.hide()
				})
			}
			"show" == this.state ? c.show("_init") : c.hide("_init"), this.settings.clickHideMode && (c._$maskLayer.on($.isPC ? "click" : "tap", function() {
				c.hide()
			}), 1 == c.settings.clickHideMode && c._$maskLayer.children().on($.isPC ? "click" : "tap", function(a) {
				a.stopPropagation()
			}))
		};
			...
			switch (arguments.length > 0 && "string" == typeof arguments[0] && (c = arguments[0]), c) {
				case "init":
					this.each(function(c, d) {
						var e = $(d),
							f = new a(e, b);
						e.data("plugin_maskLayer", f)
					});
					break;
				case "getPluginObject":
					return this.data("plugin_maskLayer");
				default:
					var d = this.data("plugin_maskLayer"),
						e = d[c];
					if (e) {
						var f = [];
						arguments.length > 1 && (f = arguments[1]), e.apply(d, f)
					}
			}
			return this
		}
	...
	c.exports = e
}), define("units/weixin", [], function(a, b, c) {
	c.exports = Zepto,
	function(a) {
		a.fn.wx = function(b) {
			function c(a, b) {
	...
	var e = new d;
	c.exports = e
}), define("units/ylmap", [], function(a, b, c) {
	c.exports = Zepto,
	function(a) {
