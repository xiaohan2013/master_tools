				...
				var MeteorShower = function($item, options) {
					...
					for ( var left, top, styleIndex, delay, duration, html = "", i = 0; i < this.settings.starCount; i++)
						left = (640 * Math.random()).toFixed(2),
								top = (600 * Math.random()).toFixed(2),
								delay = Math.random().toFixed(2),
								duration = (1 + 4 * Math.random()).toFixed(),
								styleIndex = Math.round(1 + 3 * Math.random()),
								html += '<i class="star style' + styleIndex
										+ '" style="left:' + left + "px; top:"
										+ top + "px; -webkit-animation-delay:"
										+ delay + "s; -webkit-animation: star "
										+ duration + 's linear infinite;"></i>';
					for ( var i = 0; i < this.settings.meteorCount; i++)
						left = (800 * Math.random() - 280).toFixed(2),
								...
								duration = (1.2 + 2.8 * Math.random())
										.toFixed(), styleIndex = Math
										.round(1 + 3 * Math.random()),
								html += '<i class="meteor style' + styleIndex
										+ '" style="left:' + left + "px; top:"
										+ top + "px; -webkit-animation-delay:"
										+ delay
										+ "s; -webkit-animation: meteor "
										+ duration + 's linear infinite;"></i>';
				...
				$.fn.meteorShower = function() {
					...
					case "init":
						this.each(function(i, item) {
							var $item = $(item), pluginObj = new MeteorShower(
									$item);
