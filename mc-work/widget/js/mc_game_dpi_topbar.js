...
(function() {
	...
	var html = [
		...
		'</div>',
		'<div class="ad-bottom-btn"  data-action="ads" data-label="点击关注" data-id="$!socialGame.id">',
...
$(document).ready(function() {
	if (mc && mc.ads) {
		$('body').on('click touchstart', '#btn-ad-arrow', function(e) {
			e.preventDefault();
			var obj = this;
			$('.gm-page-ad .mask').toggle();
			$('.ad-content').slideToggle(function() {
				$('.ad-top-text').toggle();
				$('.border-shadow').toggle();
				if ($(this).css("display") == 'block') {
					$(obj).attr('src', 'http://style.mc2113.com/dist/game/christmas/static/img/btn_ad_arrow_up_v3.png').attr("width", "200");
				} else {
					$(obj).attr('src', 'http://style.mc2113.com/dist/game/christmas/static/img/btn_ad_arrow_down.png').attr("width", "240");
				}
			});
		});

		$('body').on('click', '.gm-page-ad .ad-bottom-btn', function() {
			if ($(this).data("action") && $(this).data("label")) {
				try {
					_paq.push(['trackEvent', $(this).data("action"), $(this).data("label") + "_" + $(this).data("id")]);
				} catch (e) {}
			}
			window.location.href = mc.ads.link;
		});

		$('#ad-img').attr('src', mc.ads.pic);
	...
	MCUI.TOPBAR.refreshTipTitle = function(playCnt) {
		var tip = "您还有" + playCnt + "次游戏机会," + mc.ads.tipTitle;
